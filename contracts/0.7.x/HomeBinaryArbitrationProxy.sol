...
contract HomeBinaryArbitrationProxy is IHomeBinaryArbitrationProxy {
    ...
    /// @dev The address of the home governor proxy contract, which must only allow calls coming from the real governor on the foreign chain. TRUSTED.
    address public governor = msg.sender;

    /// @dev Address of the counter-party proxy on the Foreign Chain. TRUSTED.
    ICrossChainArbitrable public xKlerosLiquid;
     ...
     * @dev The contract will still require initialization before being usable.  
     * @param _amb ArbitraryMessageBridge contract address.
     * @param _xKlerosLiquid ArbitraryMessageBridge contract address.
     */
    constructor(IAMB _amb, ICrossChainArbitrable _xKlerosLiquid) {
        amb = _amb;
        xKlerosLiquid = _xKlerosLiquid;
    }
     ...
     * @param _extraData The address of the arbitrable contract. UNTRUSTED.
     * @param _homeDisputeID The ID of the arbitrable item on the arbitrable contract.
     * @param _plaintiff The address of the dispute creator.
     */
    function receiveDisputeRequest(
        bytes memory _extraData,
        uint256 _homeDisputeID,
        address _plaintiff
    ) external override onlyForeignProxy {
        ArbitrableItem storage arbitrableItem = arbitrableItems[_plaintiff][_homeDisputeID];
        require(arbitrableItem.status == Status.None, "Dispute request already exists");

        try xKlerosLiquid.notifyDisputeRequest(_arbitrableItemID, _extraData) {
